...
这些视图都固定增加“周期状态包含当前周期”的条件；“本周期已提交”同时包含“已提交”和“补卡”，“本周期待提交”同时包含“待核验”和“未提交”。补卡成员仍可在独立视图中查看。除“当前作业周期”外，其他统计视图都排除“人工状态=不参与统计”。切换作业时由机器人更新周期状态，不使用 Today、Yesterday 一类相对日期筛选。
...
```bash
COURSE_PHASES_JSON=[{"name":"技术周","start_date":"2026-08-17","end_date":"2026-08-26","cycle_days":2,"publish_hour":10,"publish_minute":0,"due_hour":20,"due_minute":0},{"name":"视频周","start_date":"2026-08-28","end_date":"","cycle_days":1,"publish_hour":8,"publish_minute":0,"due_day_offset":1,"due_hour":12,"due_minute":0,"makeup_day_offset":0,"makeup_hour":22,"makeup_minute":0}]
```

对应逻辑是：

- 技术周保留原规则：第一天 10:00 发布，第二天 20:00 截止；
- 视频周每天 08:00 开始统计，当天布置的作业在次日 12:00 正常截止，并在新阶段内从第 1 次重新编号；
- 视频周次日 12:00 后至 22:00 前记为补卡，22:00 起仍未交记为旷卡；
- 同一周期统一使用周期第一天作为记录日期；
...
# 不另发中间“最终汇总”，避免和 22:00 补交汇总重复
FINAL_STATUS_ENABLED=false
FINAL_STATUS_HOUR=22
FINAL_STATUS_MINUTE=0

# 17:00 已有未交名单，不再另发一条补交 @ 提醒
MAKEUP_REMINDER_ENABLED=false
MAKEUP_REMINDER_HOUR=17
MAKEUP_REMINDER_MINUTE=0

# 旧阶段兼容值；视频周在 COURSE_PHASES_JSON 中覆盖为截止当日 22:00
MAKEUP_DEADLINE_HOUR=12
MAKEUP_DEADLINE_MINUTE=0
MAKEUP_SUMMARY_ENABLED=true
MAKEUP_SUMMARY_HOUR=22
...
关闭普通日报不会影响作业催交、未交名单、补交汇总和群内手动查询。

## 十、作业和复盘如何判定

### 作业

机器人优先按当期作业话题统计。`已完成`、`已提交`、`作业提交` 都只是作业声明，声明本身不等于作品。视频周必须能关联到以下任一真实作品证据：
...
声明和作品可在同一条消息、同一话题，或同一发送人前后 10 分钟内分开发。有多个相邻作业声明时，作品只归给时间最近的那一次。只有声明但没有作品时先记为“待核验”；补上作品后按作品真实时间改为正常提交或补卡，补交截止后仍无证据才记为未提交。
...
# 正常截止日 17:00 发未交名单；最终汇总由 22:00 补交汇总承担
MISSING_LIST_ENABLED=true
MISSING_LIST_HOUR=17
MISSING_LIST_MINUTE=0
FINAL_STATUS_ENABLED=false
...
# 视频周在正常截止当日 22:00 结束补交并发最终汇总
# 17:00 已有未交名单，因此不再另发一条补交 @ 提醒
MAKEUP_REMINDER_ENABLED=false
MAKEUP_REMINDER_HOUR=17
MAKEUP_REMINDER_MINUTE=0
# 全局值作为旧阶段兼容配置；视频周的 22:00 由 COURSE_PHASES_JSON 覆盖
MAKEUP_DEADLINE_HOUR=12
MAKEUP_DEADLINE_MINUTE=0
MAKEUP_SUMMARY_ENABLED=true
MAKEUP_SUMMARY_HOUR=22
...
# 单次作业延期（可选）：键为作业日期，值为上海时间截止点；不会改变其他作业的截止时间
# 例如：ASSIGNMENT_DEADLINE_OVERRIDES_JSON={"2026-08-17":"2026-08-18 14:00"}
ASSIGNMENT_DEADLINE_OVERRIDES_JSON=

# 特殊排期与课程内容路由：支持缓冲日、跨线作业和提前开放任务。
# 文件不含密钥，可直接随源码部署；不需要特殊排期时留空。
ASSIGNMENT_ROUTES_PATH=./config/assignment_routes.json
...
```json
{
  ...
  "fields": [
    ...
    },
    {
      ...
      "options": [
        ...
        {"name": "补卡", "hue": "Orange", "lightness": "Light"},
        {"name": "待核验", "hue": "Yellow", "lightness": "Light"},
        {"name": "未提交", "hue": "Red", "lightness": "Light"}
      ],
      "description": "合并机器人识别与人工状态后的有效作业状态；只有提交声明、尚未关联到作品证据时为“待核验”。",
      "managed_by": "bot"
    },
    {
      "name": "复盘状态",
  ...
  "views": [
    ...
    },
    {
      "name": "本周期待提交",
      "type": "grid",
      "filter": {"logic": "and", "conditions": [["周期状态", "intersects", ["当前周期"]], ["作业状态", "intersects", ["待核验", "未提交"]], ["人工状态", "disjoint", ["不参与统计"]]]},
      "sort": {"sort_config": [{"field": "组员姓名", "desc": false}]},
...
Environment=FINAL_STATUS_ENABLED=false
Environment=FINAL_STATUS_HOUR=22
Environment=FINAL_STATUS_MINUTE=0
Environment=MAKEUP_REMINDER_ENABLED=false
...
Environment=MAKEUP_SUMMARY_HOUR=22
Environment=MAKEUP_SUMMARY_MINUTE=0
Environment='COURSE_PHASES_JSON=[{"name":"技术周","start_date":"2026-08-17","end_date":"2026-08-26","cycle_days":2,"publish_hour":10,"publish_minute":0,"due_hour":20,"due_minute":0},{"name":"视频周","start_date":"2026-08-28","end_date":"","cycle_days":1,"publish_hour":8,"publish_minute":0,"due_day_offset":1,"due_hour":12,"due_minute":0,"makeup_day_offset":0,"makeup_hour":22,"makeup_minute":0}]'
...
class GroupSummaryService:
    ...
    def _completion_markers_for_day(
        ...
        *,
        ...
        route = self.settings.assignment_route_for_text(text, marker_reference_day)
    ...
    def _marker_report_date(self, marker_day: date, label: str, text: str = "") -> str:
        ...
        """
        if text and (
            route := self.settings.assignment_route_for_text(text, marker_day)
        ) is not None:
