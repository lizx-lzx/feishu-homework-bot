...
代改命令只向 `leader_member_ids` 中的真实发送人开放。机器人只写多维表格的“人工状态”字段，同时在 SQLite 的 `attendance_overrides` 表中保留操作人、目标成员、作业周期、状态和消息 ID。普通成员发同样的命令会被拒绝。打开链接后能否直接编辑，仍由该多维表格本身的飞书分享权限决定；即使成员只有查看权限，已授权组长仍可通过机器人命令完成代改。
