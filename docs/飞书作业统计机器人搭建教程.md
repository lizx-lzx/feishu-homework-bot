...
如果开启 `SOCIAL_CHAT_ENABLED`，MiniMax 在成员直接 @ 机器人时可以回答群规则、作业、AI 工具、项目实操和其他低风险普通问题，不再用“只支持作业统计”的旧兜底截断。催交提醒等机器人自身规则由程序按当前真实配置回答，不交给模型猜。同时开启 `SOCIAL_CHAT_PROACTIVE_ENABLED` 后，每条新消息会先经过确定性信号筛选，只有明确求助、报错卡住或小进展才会交给模型决定 `silent / react / reply`。主动说话默认在 23:00—08:00 静默，每群冷却 10 分钟，每小时最多 3 次。模型回复只能发文字或表情，不能调用任何改表路径。
