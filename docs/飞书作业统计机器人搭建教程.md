...
如果开启 `SOCIAL_CHAT_ENABLED`，MiniMax 还可以在成员 @ 机器人时进行作业、AI 工具和项目实操范围内的简短对话。同时开启 `SOCIAL_CHAT_PROACTIVE_ENABLED` 后，每条新消息会先经过确定性信号筛选，只有明确求助、报错卡住或小进展才会交给模型决定 `silent / react / reply`。主动说话默认在 23:00—08:00 静默，每群冷却 10 分钟，每小时最多 3 次。模型回复只能发文字或表情，不能调用任何改表路径。
